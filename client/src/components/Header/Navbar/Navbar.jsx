...
const Navbar = () => {
  ...
  const [name, setName] = useState(initialSate);
        ...
        <NavbarItem>
          <NavbarItemContainer>
            ...
            <NavbarSubMenu className="sub">
                ...
                {dataImage.map((item) => (
                  <NavbarSubItemImage key={item.id}>
                    <SubItemImageLink to={item.link}>
                      ...
                      <SubItemImageTitle>{item.title}</SubItemImageTitle>
                    </SubItemImageLink>
                  </NavbarSubItemImage>
                ))}
              </NavbarItemImageWrapper>
              <SubItemFooter>
                <SubItemDescription>
                  People call me <SubItemNameBrand>LV7</SubItemNameBrand> !
                </SubItemDescription>
              </SubItemFooter>
            </NavbarSubMenu>
          </NavbarItemContainer>
          ...
          <NavbarItemContainer>
            ...
            <NavbarSubMenu className="sub">
              <NavbarItemWrapper>
                {data.map((dt) =>
                  dt.separate ? (
                    <NavbarSubItem key={dt.id} separate="dash" />
                  ) : (
                    <NavbarSubItem key={dt.id}>
                      <SubItemLink to="#">
                        <SubItemHighlight>{dt.highlight}</SubItemHighlight>
                      </SubItemLink>
                      {dt.status && (
                        <>
                          <SubItemBlank />
                          {dt.status.map((st) => (
                            <SubItemLink key={st.id} to={st.link}>
                              <SubItemName>{st.value}</SubItemName>
                            </SubItemLink>
                          ))}
                        </>
                      )}
                      {dt.list.map((lt) => (
                        <Fragment key={lt.id}>
                          ...
                          <SubItemLink to={lt.link}>
                            <SubItemTitle>{lt.title}</SubItemTitle>
                          </SubItemLink>
                          {lt.items.map((item) => (
                            <SubItemLink key={item.id} to={item.link}>
                              <SubItemName>{item.value}</SubItemName>
                            </SubItemLink>
                          ))}
                        </Fragment>
                      ))}
                      <SubItemBlank />
                      {dt.footer && (
                        <SubItemLink to={dt.footer.link}>
                          <SubItemTitle>{dt.footer.value}</SubItemTitle>
                        </SubItemLink>
                      )}
                    </NavbarSubItem>
                  )
